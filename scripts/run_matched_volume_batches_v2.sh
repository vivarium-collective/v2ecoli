#!/usr/bin/env bash
# Extension driver: seed pairs (18,19), (20,21), (22,23) to push above 100 Δt pairs.
# Runs no-block and chromblock25 for each pair, extracts volume, deletes parquets.
#
# Usage: nohup bash scripts/run_matched_volume_batches_v2.sh > out/matched_volume_driver_v2.log 2>&1 &
...
KDMIN=3
FRAC=0.25
INTERSECTION_PAIRS="18,19 20,21 22,23"
...
for pair in $INTERSECTION_PAIRS; do
    ...
    bash scripts/run_basal_soft_kd.sh softgrad "$RAMP" $KDMIN "$A" "$B" > "out/matched_noblock_v2_seeds${A}and${B}_launcher.log" 2>&1
    log "no-block seeds $A, $B done."
    extract_config noblock "$A" "$B"
    cleanup_config noblock "$A" "$B"

    log "launching chromblock25 seeds $A, $B..."
    bash scripts/run_basal_softgrad_chromblock.sh $FRAC "$A" "$B" > "out/matched_chromblock25_v2_seeds${A}and${B}_launcher.log" 2>&1
...
log "==== ALL EXTENSION BATCHES DONE ===="
