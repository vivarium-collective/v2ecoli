...
from v2ecoli.processes.parca.wholecell.utils.unit_struct_array import UnitStructArray
from v2ecoli.processes.parca.wholecell.utils.units import g, mol
import numpy as np
...
class Test_unit_struct_array(unittest.TestCase):
    ...
    def test_init(self):
        with self.assertRaisesRegex(
            ...
            UnitStructArray(1.0, {"hello": "goodbye"})
    ...
    def test_field(self):
        self.assertEqual(self.us_array["id"].tolist(), self.struct_array["id"].tolist())

        self.assertTrue((self.us_array["mass"] == g * self.struct_array["mass"]).all())
    ...
    def test_getItem_slice(self):
        self.assertEqual(
            self.us_array[:1], UnitStructArray(self.struct_array[:1], self.units)
        )
    ...
    def test_getItem_indicies(self):
        ...
        self.assertEqual(
            self.us_array[index], UnitStructArray(self.struct_array[index], self.units)
        )
    ...
    def test_setItem_quantity_with_units(self):
        self.us_array["mass"] = g * np.array([1.0, 2.0, 3.0])
        self.assertTrue((self.us_array["mass"] == g * np.array([1.0, 2.0, 3.0])).all())

        with self.assertRaisesRegex(Exception, "Units do not match!\n"):
            self.us_array["mass"] = mol * np.array([1.0, 2.0, 3.0])
