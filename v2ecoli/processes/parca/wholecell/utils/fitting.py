...
def countsFromMassAndExpression(
    mass: float,
    ...
    """
    Args:
        mass: Total mass you want counts to sum to
...
def masses_and_counts_for_homeostatic_target(
    ...
    cell_volume = dry_mass_of_non_small_molecules / (
...
def calcProteinDistribution(sim_data):
    return normalize(
        sim_data.process.transcription.rna_data["expression"][
    ...
    for x_name in x_fun:
        ...
        for y_name in y_fun:
            ...
            if verbose and abs_r > verbose:
                print(
                    "{} {}: {:.3f} {:.1e}".format(
                        x_name, y_name, result.rvalue, result.pvalue
                    )
                )

    if verbose:
        print(
            f"Selected functions (r={best_r:.3f} p={best_p:.1e}):\n\tx: {x_transform}\n\ty: {y_transform}"
        )
