...
- **Reference-side vEcoli pin (required for the reference arm).** The reference (`vecoli`) arm needs `$V2E_VECOLI_DIR` pointed at a vEcoli checkout, but **not just any HEAD**: upstream CovertLab commit `14f04a3f` changed `get_monomer_counts_listener_config` (`ecoli/library/sim_data.py`) from `.molecule_names` (41 rows, matching the stoich matrix) to `.modified_molecules` (45), so the `vecoli` composite fails at `ecoli/processes/listeners/monomer_counts.py` with `ValueError: operands could not be broadcast together with shapes (45,) (41,)`. A fresh HEAD-compatible ParCa rebuild does **not** fix this — the mismatch is in the listener config, not the cache. Until it is fixed/reported upstream, pin `$V2E_VECOLI_DIR` to a vEcoli commit **before `14f04a3f`** (or revert that one line locally). This is an external-repo runtime requirement, not a v2ecoli code change — it does not gate this build's hermetic tests.
