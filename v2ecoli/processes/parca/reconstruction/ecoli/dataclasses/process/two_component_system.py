...
from v2ecoli.processes.parca.wholecell.utils import build_ode
from v2ecoli.processes.parca.wholecell.utils import data
...
class TwoComponentSystem(object):
    def __init__(self, raw_data, sim_data):
        ...
        signalingTemplate = {
            1: [
                "POS-LIGAND-BOUND-HK-PHOSPHORYLATION_RXN",
        ...
        for reactionIndex, reaction in enumerate(
            raw_data.two_component_system_templates
        ):
                ...
                for molecule in reactionTemplate[reaction]["stoichiometry"]:
                    ...
                    if molecule["molecule"] in system["molecules"]:
                        moleculeName = "{}[{}]".format(
                            ...
                            sim_data.getter.get_compartment(
                                system["molecules"][molecule["molecule"]]
                            )[0],
                        )
                    ...
                    if (
                        str(molecule["molecule"]) in ["HK", "RR", "ATP"]
                        and moleculeName not in independentMolecules
                    ):
                        ...
                        if str(molecule["molecule"]) == "RR":
                            activeTF = "{}[{}]".format(
                                system["molecules"]["PHOSPHO-RR"], molecule["location"]
                            )
                    ...
                    if system["orientation"] == 1:
                        if (
                            str(molecule["molecule"]) == "HK-LIGAND"
                            and moleculeName not in independentMolecules
                        ):
                            independentMolecules.append(moleculeName)
                            independent_molecule_indexes.append(moleculeIndex)
                    ...
                    # Find molecular mass
                    molecularMass = sim_data.getter.get_mass(moleculeName).asNumber(
                        units.g / units.mol
                    )
        ...
        self._stoichMatrixV = np.array(
            stoichMatrixV
        )  # arrary of stoichometric coefficients
        ...
        self.independent_molecules_atp_index = np.where(
        ...
        self.atp_reaction_reactant_mask = (
            self.dependency_matrix[:, self.independent_molecules_atp_index] < 0
        )
    ...
    def __getstate__(self):
        ...
        return data.dissoc_strict(
            ...
            (
                ...
                "derivatives_parca_symbolic",
                "derivatives_parca_jacobian_symbolic",
                "_rates",
                "_rates_jacobian",
                "derivatives_parca",
                "derivatives_parca_jacobian",
                "dependency_matrix",
                "_stoich_matrix",
            ),
        )
        ...
        for row in modifiedFormsMonomers:
            ...
            if molecule_and_location in tcsMolecules:
                ...
                for subunit in row["subunits"]:
                    ...
                    # Correct a compartment-tag mismatch by canonicalizing to the
                    # bulk-data compartment. Upstream vEcoli *raises* here to force
                    # a fix in reconstruction/ecoli/flat/modified_proteins.tsv; in
                    # v2ecoli that flat file ships in the pinned `ecoli_sources`
                    # package (not editable from this repo), so we warn and
                    # canonicalize instead — which IS the substantive fix: the
                    # subunit is then found in the bulk pool and its counts are
                    # correctly recycled (the old code silently dropped the
                    # mis-tagged subunit).
                    if original_compartment != expected_compartment:
                        warnings.warn(
                            f"Compartment tag mismatch for subunit "
                            f"'{subunit['monomer']}' in complex "
                            f"'{molecule_and_location}': modified_proteins.tsv has "
                            f"'[{original_compartment}]' but bulk molecule data has "
                            f"'[{expected_compartment}]'. Canonicalizing to "
                            f"'[{expected_compartment}]'."
                        )
    ...
    def _populate_derivative_and_jacobian(self):
        ...
        self._rates_jacobian = build_ode.derivatives_jacobian(
            self.symbolic_rates_jacobian
        )
        self._stoich_matrix = (
            self.stoich_matrix()
        )  # Matrix is small and can be cached for derivatives

        # WORKAROUND: Avoid Numba LoweringError JIT-compiling these functions:
        self.derivatives_parca = build_ode.derivatives(self.derivatives_parca_symbolic)[
            0
        ]
        self.derivatives_parca_jacobian = build_ode.derivatives_jacobian(
            self.derivatives_parca_jacobian_symbolic
        )[0]
    ...
    def molecules_to_next_time_step(
        ...
        independentMoleculesCounts = np.round(
            dYMolecules[self.independent_molecule_indexes]
        )
        ...
        independentMoleculesCounts[self.independent_molecules_atp_index] = np.fmin(
            ...
            + independentMoleculesCounts[
                (self.independent_molecules_atp_index + 1) :
            ].sum(),
            max_atp_rxns,
        )
        ...
        while np.any(final_molecules < 0):
            ...
            rxns = (
                stoich[mol_idx, :] < 0
            )  # reactions that consume the molecule that has been depleted

            # Get products of reactions to turn back into reactants
            consuming_stoich = stoich[:, rxns]
            consuming_stoich[consuming_stoich < 0] = (
                0  # exclude molecules that are also consumed in these reactions
            )
            consuming_stoich[consuming_stoich.sum(axis=1) > 1] = (
                0  # exclude molecules that are shared between reactions
            )
            ...
            if iteration > stoich.shape[1]:
                raise ValueError(
                    "Could not get positive molecule counts for {} in two_component_system".format(
                        self.molecule_names[mol_idx]
                    )
                )
    ...
    def molecules_to_ss(self, moleculeCounts, cellVolume, nAvogadro, timeStepSec=1e20):
        ...
        y = scipy.integrate.odeint(
            self.derivatives_parca,
            y_init,
            t=[0, timeStepSec],
            Dfun=self.derivatives_parca_jacobian,
        )
        ...
        # Calculate changes in molecule counts for all molecules
        allMoleculesChanges = np.dot(self.dependency_matrix, independentMoleculesCounts)
        ...
        for endIndex in [x.start() for x in re.finditer("-", templateName)]:
            if templateName[startIndex:endIndex] in systemMolecules:
                reactionName = reactionName.replace(
                    templateName[startIndex:endIndex],
                    str(systemMolecules[templateName[startIndex:endIndex]]),
                )
    ...
    def _make_dependency_matrix(self):
        ...
        for independentMoleculeIndex, independentMoleculeId in enumerate(
            self.independent_molecule_indexes
        ):
            ...
            if self.molecule_names[independentMoleculeId] == "ATP[c]":
                ...
                dependentMoleculeId = molecule_name_to_index[
                    self.independent_to_dependent_molecules[
                        self.molecule_names[independentMoleculeId]
                    ]
                ]
