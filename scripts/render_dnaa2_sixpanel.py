...
def _load_seed(run_dir: str, seed: int):
    ...
    def _idx(mol: str) -> int:
        try:
            ...
            raise SystemExit(
                f"bulk species {mol!r} absent from run output — the clean "
                f"baseline (post-bf8b82e) must emit the DnaA nucleotide forms")

    # DnaA nucleotide forms are read straight from the BULK store. The bespoke
    # dnaA_cycle listener (and the DnaaAdpRelease/DnaaIntrinsicHydrolysis steps)
    # were removed when we adopted Haochen's integrate_dt equilibrium (bf8b82e):
    # the equilibrium machinery now interconverts these three bulk molecules, so
    # we derive total/fractions from the molecule counts themselves — more
    # faithful than a derived listener, and works on the stock baseline.
    #   apo-DnaA = PD03831[c]; DnaA-ATP = MONOMER0-160[c]; DnaA-ADP = MONOMER0-4565[c]
    atp_i, adp_i = _idx("ATP[c]"), _idx("ADP[c]")
    dapo_i, datp_i, dadp_i = _idx("PD03831[c]"), _idx("MONOMER0-160[c]"), _idx("MONOMER0-4565[c]")
    ...
    df = (pl.scan_parquet(files, hive_partitioning=True)
          # Follow ONLY the canonical all-zeros lineage (the followed daughter):
          # agent_ids 0, 00, 000, ... — the sibling line (…001) emits a few
          # post-division ticks that would otherwise interleave into gens 3+.
          .filter(pl.col("agent_id").cast(pl.Utf8).str.contains("^0+$"))
          .select(keep + [
              bc.list.get(datp_i).alias("listeners__dnaA_cycle__atp_count"),
              bc.list.get(dadp_i).alias("listeners__dnaA_cycle__adp_count"),
              bc.list.get(dapo_i).alias("listeners__dnaA_cycle__apo_count"),
              bc.list.get(atp_i).alias("bulk_atp"),
              bc.list.get(adp_i).alias("bulk_adp")])
          .sort(["generation", "global_time"]).collect())
    # Drop <5-min daughter-stub partitions (the final division spawns a sliver
    # generation that is not a real cell cycle — keep it out of the count + plot).
    _dur = (df.group_by("generation")
              .agg(((pl.col("global_time").max() - pl.col("global_time").min())
                    / 60.0).alias("_dur")))
    _real = _dur.filter(pl.col("_dur") >= 5.0)["generation"].to_list()
    df = df.filter(pl.col("generation").is_in(_real))
    # total + fractions derived from the three bulk DnaA species
    tot = (pl.col("listeners__dnaA_cycle__atp_count")
           + pl.col("listeners__dnaA_cycle__adp_count")
           + pl.col("listeners__dnaA_cycle__apo_count"))
    df = df.with_columns(tot.alias("listeners__dnaA_cycle__total"))
    df = df.with_columns([
        (pl.col("listeners__dnaA_cycle__atp_count")
         / pl.col("listeners__dnaA_cycle__total")).alias("listeners__dnaA_cycle__atp_fraction"),
        (pl.col("listeners__dnaA_cycle__adp_count")
         / pl.col("listeners__dnaA_cycle__total")).alias("listeners__dnaA_cycle__adp_fraction"),
        (pl.col("listeners__dnaA_cycle__apo_count")
         / pl.col("listeners__dnaA_cycle__total")).alias("listeners__dnaA_cycle__apo_fraction")])
...
def render(run_dir: str, seed: int, step: int, out_dir: str) -> str:
    ...
    # Label the generation number for each segment (Rashmi 2026-05-30: "label the
    # number of generations"). Segment edges = [0, *bounds, end]; centre a "gen N"
    # tag at the top of the top panel. x in data coords, y in axes fraction.
    edges = [0.0] + list(bounds) + [float(x.max())]
    for i in range(len(edges) - 1):
        mid = (edges[i] + edges[i + 1]) / 2.0
        ax[0].text(mid, 0.93, f"gen {i + 1}", transform=ax[0].get_xaxis_transform(),
                   ha="center", va="top", fontsize=8, fontweight="bold", color="0.30")
