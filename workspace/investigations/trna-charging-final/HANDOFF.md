...
- **Session 7 (next):** Task #2e (`get_elongation_rate`, ~159 LOC, lines 464–622). Pure deterministic — no RNG, no mutating writes to inputs. Binary search over `col` where the elongation rate matches a target. Use `@njit` (no Python orchestration needed). Then port the upstream 580-line companion test `wholecell/tests/utils/test_trna_charging.py` into `tests/test_trna_charging_kernel_companion.py` (or merge into the existing kernel test); most assertions wire straight to the new module. Must pass under `pytest -m 'not sim'`.
