...
class PolypeptideInitiationStep(_SafeInvokeMixin, Step):
    """Polypeptide Initiation — merged single-step process."""
    ...
    def initialize(self, config):
        ...
        self.translation_efficiencies = params["translation_efficiencies"]
        self.active_ribosome_fraction = params["active_ribosome_fraction"]
        self.ribosome_elongation_rates_dict = params["elongation_rates"]
        self.variable_elongation = params["variable_elongation"]
        self.make_elongation_rates = params["make_elongation_rates"]

        self.rna_id_to_cistron_indexes = params["rna_id_to_cistron_indexes"]
        self.cistron_start_end_pos_in_tu = params[
            "cistron_start_end_pos_in_tu"
        ]
        self.tu_ids = params["tu_ids"]
        ...
        self.active_ribosome_footprint_size = (
            params["active_ribosome_footprint_size"] / 3
        )

        # Get mapping from cistrons to protein monomers and TUs
        self.cistron_to_monomer_mapping = params["cistron_to_monomer_mapping"]
        self.cistron_tu_mapping_matrix = params["cistron_tu_mapping_matrix"]
        self.monomer_index_to_cistron_index = params[
            "monomer_index_to_cistron_index"
        ]
        self.monomer_index_to_tu_indexes = params[
            "monomer_index_to_tu_indexes"
        ]
        ...
        self.monomer_ids = params["monomer_ids"]
    ...
    def update(self, state, interval=None):
        ...
        # ==================================================================
        # Phase 1: Requester logic — compute elongation rates and fractions
        # ==================================================================

        current_media_id = state["environment"]["media_id"]

        fracActiveRibosome = self.active_ribosome_fraction[current_media_id]
        ...
        # If the ribosome elongation rate is zero (first timestep), use dict
        if ribosomeElongationRate == 0:
            ribosomeElongationRate = self.ribosome_elongation_rates_dict[
                current_media_id
        ...
        # ==================================================================
        # Phase 2: Evolver logic — activate ribosomes using actual counts
        # ==================================================================

        # Calculate number of ribosomes that could potentially be initialized
        # based on actual counts of free 30S and 50S subunits (no allocation)
        ...
        # Calculate initiation probabilities for ribosomes
        ...
        if n_ribosomes_to_activate == 0:
            ...
            # allowed from the known ribosome footprint sizes
            max_p = (
                ribosomeElongationRate
                / self.active_ribosome_footprint_size
                * (units.s)
            ...
            # Initialize flag to record if the number of ribosomes activated at
            # this time step needed to be reduced to prevent overcrowding
            ...
            while np.any(protein_init_prob > max_p_per_protein):
                if protein_init_prob[~is_overcrowded].sum() != 0:
                    ...
                    # Update maximum probabilities based on new number of
                    # activated ribosomes.
                    max_p = (
                        ribosomeElongationRate
                        / self.active_ribosome_footprint_size
            ...
            # Sample multinomial distribution to determine which mRNAs have
            # full 70S ribosomes initialized on them
