...
def get_in(d, path, default=None):
    """Traverse nested dict *d* along *path*, returning *default* on miss."""
    for key in path:
...
def assoc_path(d, path, value):
    """Set *value* at *path* in nested dict *d*, creating dicts as needed."""
    for key in path[:-1]:
        d = d.setdefault(key, {})
    d[path[-1]] = value
...
PARAMETER_DICT = {
    ...
    "cell_wall": {
        ...
        "disaccharide_height": Parameter(
            1.03 * units.nm, "Vollmer, W., & Hoeltje, J.-V. (2004)."
        ),
