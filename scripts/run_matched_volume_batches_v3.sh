#!/usr/bin/env bash
# Final push: seeds 8, 9 (previously crashed for no-block) and 12, 15 as backups.
...
extract_config() {
    ...
    for S in "$seed_a" "$seed_b"; do
        ...
        if [ -d "$pq_root" ]; then
            ...
            log "SKIP extract $config seed $S — no parquet found (likely crashed)"
        fi
    done
}
...
for pair in $INTERSECTION_PAIRS; do
    ...
    bash scripts/run_basal_soft_kd.sh softgrad "$RAMP" $KDMIN "$A" "$B" > "out/matched_noblock_v3_seeds${A}and${B}_launcher.log" 2>&1
    log "no-block seeds $A, $B done."
    extract_config noblock "$A" "$B"
    cleanup_config noblock "$A" "$B"

    log "launching chromblock25 seeds $A, $B..."
    bash scripts/run_basal_softgrad_chromblock.sh $FRAC "$A" "$B" > "out/matched_chromblock25_v3_seeds${A}and${B}_launcher.log" 2>&1
