...
# (`nitrogen_in_mM` pinned at exactly the initial pool), and the elemental
# residuals broke at that point -- carbon -0.7% -> +12.3%, nitrogen +7.4% ->
# -11.8% -- because growth continued past exhaustion while the ledger's inputs
# were capped. Sizing N off the recipe leaves an OD10 run on a knife edge where
# a slightly faster seed hits the wall.
#
...
def _reactor_store(
    ...
    return {
        ...
        # Elemental-closure diagnostics (coupler-only writer). OVERWRITE, not
        # additive: each is recomputed from cumulative totals every tick, so an
        # additive apply would sum a running residual into nonsense.
        # ⚠ mbp-03/04/05 declare `reactor.diagnostics.*` readouts; before this
        # the subtree did not exist at all (zero of 1222 leaves matched
        # "diagnostic"), so those readouts resolved to nothing and mbp-04's
        # Phase-2 acceptance could not be evaluated as written.
        DIAGNOSTICS_LEAF: {leaf: {"_type": "overwrite[float]", "_default": 0.0}
...
def add_reactor_coupling(
    ...
    *,
    ...
    state[REACTOR_STORE_NAME] = _reactor_store(bird_cfg, initial_glucose_mM, initial_ammonium_mM)
    ...
    coupler_edge["_outputs"]["reactor"] = {
        ...
        DIAGNOSTICS_LEAF: {leaf: "overwrite[float]" for leaf in DIAGNOSTIC_LEAVES},
        "acetate_mM":   "float",
        "lactate_mM":   "float",
        "formate_mM":   "float",
        "ethanol_mM":   "float",
