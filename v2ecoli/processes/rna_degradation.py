...
class RnaDegradationStep(_SafeInvokeMixin, Step):
    """RNA Degradation — merged single-step process."""
    ...
    def initialize(self, config):
        ...
        self.rna_ids = params["rna_ids"]
        self.mature_rna_ids = params["mature_rna_ids"]
        self.n_transcribed_rnas = len(self.rna_ids)
        self.mature_rna_exists = len(self.mature_rna_ids) > 0
        self.cistron_ids = params["cistron_ids"]
        self.cistron_tu_mapping_matrix = params["cistron_tu_mapping_matrix"]
        self.mature_rna_cistron_indexes = params["mature_rna_cistron_indexes"]
        self.all_rna_ids = params["all_rna_ids"]
        self.n_total_RNAs = params["n_total_RNAs"]
        ...
        self.endoRNase_ids = params["endoRNase_ids"]
        self.exoRNase_ids = params["exoRNase_ids"]
        self.kcat_exoRNase = params["kcat_exoRNase"]
        self.Kcat_endoRNases = params["Kcat_endoRNases"]

        # Load information about uncharged/charged tRNA
        self.uncharged_trna_indexes = params["uncharged_trna_indexes"]
        self.charged_trna_names = params["charged_trna_names"]

        # Load first-order RNA degradation rates
        self.rna_deg_rates = params["rna_deg_rates"]

        self.is_mRNA = params["is_mRNA"]
        self.is_rRNA = params["is_rRNA"]
        self.is_tRNA = params["is_tRNA"]
        self.is_miscRNA = params["is_miscRNA"]
        self.degrade_misc = params["degrade_misc"]

        self.rna_lengths = params["rna_lengths"]
        self.nt_counts = params["nt_counts"]

        # Build stoichiometric matrix
        self.polymerized_ntp_ids = params["polymerized_ntp_ids"]
        self.nmp_ids = params["nmp_ids"]
        self.water_id = params["water_id"]
        self.ppi_id = params["ppi_id"]
        self.proton_id = params["proton_id"]
        ...
        # Load Michaelis-Menten constants
        self.Kms = params["Kms"]
    ...
    def update(self, state, interval=None):
        ...
        # ==================================================================
        # Phase 1: Requester logic — compute what to degrade
        # ==================================================================

        # Compute factor that convert counts into concentration
        ...
        # Get total counts of RNAs including free rRNAs, uncharged and charged
        # tRNAs, and active (translatable) unique mRNAs
        ...
        n_total_mrnas_to_degrade = self._calculate_total_n_to_degrade(
            timestep, mrna_specificity, total_kcat_endoRNase
        )
        n_total_trnas_to_degrade = self._calculate_total_n_to_degrade(
            timestep, trna_specificity, total_kcat_endoRNase
        )
        n_total_rrnas_to_degrade = self._calculate_total_n_to_degrade(
            timestep, rrna_specificity, total_kcat_endoRNase
        )
        ...
        # Compute degradation probabilities of each RNA
        if self.degrade_misc:
            mrna_deg_probs = (
                1.0
        ...
        # (mRNAs) are immediately deactivated but not degraded until
        # transcription is finished.
        n_bulk_RNAs_to_degrade = n_RNAs_to_degrade.copy()
        n_bulk_RNAs_to_degrade[self.is_mRNA.astype(bool)] = 0
        n_unique_RNAs_to_deactivate = n_RNAs_to_degrade.copy()
        n_unique_RNAs_to_deactivate[np.logical_not(self.is_mRNA.astype(bool))] = 0

        # Calculate unique RNAs that should be degraded at this timestep
        unique_mRNAs_to_degrade = np.logical_and(
            np.logical_not(can_translate), is_full_transcript
        )
        n_unique_RNAs_to_degrade = np.bincount(
            TU_index[unique_mRNAs_to_degrade], minlength=self.n_total_RNAs
        )

        # ==================================================================
        # Phase 2: Evolver logic — apply degradation using actual state
        # ==================================================================

        # Get vector of numbers of RNAs to degrade for each RNA species.
        # Since there is no allocation, use the computed bulk degradation counts
        # directly (capped by actual counts).
        n_degraded_bulk_RNA = np.minimum(
            n_bulk_RNAs_to_degrade,
            counts(state["bulk"], self.bulk_rnas_idx),
        )
        n_degraded_unique_RNA = n_unique_RNAs_to_degrade
        ...
        # Deactivate unique RNAs
        non_zero_deactivation = n_unique_RNAs_to_deactivate > 0

        for index, n_degraded in zip(
            np.arange(n_unique_RNAs_to_deactivate.size)[non_zero_deactivation],
            n_unique_RNAs_to_deactivate[non_zero_deactivation],
        ):
            ...
            if n_available == 0:
                continue
            n_to_deactivate = min(n_degraded, n_available)

            # Choose n_to_deactivate indexes randomly to deactivate
            can_translate[
                self.random_state.choice(
                    size=n_to_deactivate, a=np.where(mask)[0], replace=False
                )
            ] = False
        ...
        result = {
            "listeners": {
                "rna_degradation_listener": {
                    "fraction_active_endoRNases": np.sum(frac_endoRNase_saturated),
            ...
            "RNAs": {
                ...
                "delete": np.where(unique_mRNAs_to_degrade)[0],
            },
        ...
        if fragment_bases.sum() != 0:
            ...
            exornase_capacity = (
                n_exoRNases.sum() * self.kcat_exoRNase * (units.s * timestep)
            )
            ...
            result["listeners"]["rna_degradation_listener"]["fragment_bases_digested"] = (
