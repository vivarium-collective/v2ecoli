...
class Media(object):
    ...
    def make_recipe(self, media_id):
        ...
        if ingredient_ids:
            ...
            added_vol = recipe.get(
                "ingredients volume"
            )  # the row is a list with units.L, even an empty list is read.
    ...
    def combine_media(self, base_media, base_media_volume, mix_media, mix_media_volume):
        ...
        # intialize new_media
        new_media = {mol_id: 0.0 * CONC_UNITS for mol_id, conc in base_media.items()}
    ...
    def add_ingredients(self, base_media, base_media_volume, ingredients):
        ...
        for quantities in ingredients.values():
                    ...
                    if weight.magnitude == INF:
                        ...
                        if self.environment_molecules_fw[mol_id] is not None:
                            ...
                            raise AddIngredientsError(
                                "No fw defined for {} in environment_molecules.tsv".format(
                                    mol_id
                                )
                            )
                    else:
