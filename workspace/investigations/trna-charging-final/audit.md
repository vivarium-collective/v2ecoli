...
| `test_replication_completes_in_expected_window` | ✅ (2nd chrom at biologically reasonable time) |
...
All 7 PR-gating behavior tests pass against the new cache. No threshold adjustments needed — the new fixture is biologically valid (dry-mass doubles within [1.5, 2.5], replication completes within the [900, 1800] s window, division fires with bulk conservation, daughters are viable).

### Pre-requisite generated
First 4 tests need `out/workflow/single_cell_meta.json` (chromosome snapshots over a full cycle). Generated by running `python reports/workflow_report.py`. The trajectory itself was produced cleanly — **division at t=2491 s** (dry_mass=702.8 fg, threshold=702.8 fg, 2 chromosomes). Realtime ratio 11.9×.

### Pre-existing bugs hit in workflow_report.py during the generation
The new Task #8 cache emits `listeners.mass.*` as pint Quantities (femtogram-typed), but `workflow_report.py` had ~27 sites doing `float(mass.get('dry_mass', 0))` etc. — fails on Quantities with `DimensionalityError`. Same pattern as the Task #8 fix #4 in `kinetic_charging.py`.

Patches applied to `reports/workflow_report.py`:
1. Added `_fg(value)` helper at the top of the file: detects `hasattr(value, 'to')` → converts via `.to(units.fg).magnitude`; bare numbers pass through.
2. Bulk-replaced `float(mass.get(` → `_fg(mass.get(` (~25 occurrences) and `float(cell.get(` → `_fg(cell.get(` (~2 occurrences) via a Python sed pass.
3. One more occurrence (`float(d_mass.get('dry_mass', 0))` in `_run_daughter`) survives — surfaces only in Step 4 (daughter simulations); deferred to Task #13's full-report run.
