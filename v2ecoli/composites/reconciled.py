...
Body migrated from:
  - v2ecoli/generate_reconciled.py:build_reconciled_document — top-level document builder
  - v2ecoli/composite_reconciled.py:make_reconciled_composite — thin Composite wrapper
    (no meaningful post-processing; just passes args through to build_reconciled_document)

Helpers from v2ecoli.generate (and architecture-specific helpers from
v2ecoli.generate_reconciled) are imported rather than duplicated here.
...
# (same set that generate_reconciled.py imports)
# ---------------------------------------------------------------------------
