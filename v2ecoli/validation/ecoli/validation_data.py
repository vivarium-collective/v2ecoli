...
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.molecule_groups import (
    MoleculeGroups,
)
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.molecule_ids import (
    MoleculeIds,
)
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.constants import (
    Constants,
)
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.state.internal_state import (
    InternalState,
)
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.process.process import (
    Process,
)
...
class ValidationDataEcoli(object):
    ...
    def _add_trna_synthetase_kinetics(self, validation_data_raw, knowledge_base_raw):
        ...
        for row in validation_data_raw.trna_synthetase_kinetics:
            ...
            for i, substrate in enumerate(row["Substrate ID"]):
                ...
                if i < len(row["k_M"]):
                    self.trna_synthetase_kinetics[synthetase][substrate]["k_M"].append(
                        row["k_M"][i]
                    )
