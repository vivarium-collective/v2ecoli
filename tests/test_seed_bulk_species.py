...
adds an existing species' mass. This replaces the retired drug-flag injection
and its dedicated injection test.
"""
import numpy as np
...
def _drug_specs():
    return [
        {"id": "drugX[p]", "molar_mass_g_per_mol": 325.426},
        {"id": "drugX_hydrolyzed[p]", "molar_mass_g_per_mol": 343.426},  # +H2O
        {"id": "drugX[p]-EG10606-MONOMER[i]",
         "molar_mass_g_per_mol": 325.426, "complex_with": _PBP2},
    ]


def test_seeds_species_at_count0_with_metabolite_submass():
    out = seed_bulk_species(_synthetic_bulk(), _drug_specs())
    for name in ("drugX[p]", "drugX_hydrolyzed[p]",
                 "drugX[p]-EG10606-MONOMER[i]"):
        assert name in list(out["id"])
    free = out[out["id"] == "drugX[p]"][0]
    assert free["count"] == 0
    ...
    hydro = out[out["id"] == "drugX_hydrolyzed[p]"][0]
    assert hydro["metabolite_submass"] == 343.426 * _MOLAR_TO_FG


def test_complex_mass_is_drug_plus_partner():
    bulk = _synthetic_bulk()
    out = seed_bulk_species(bulk, _drug_specs())
    cplx = out[out["id"] == "drugX[p]-EG10606-MONOMER[i]"][0]
    free = out[out["id"] == "drugX[p]"][0]
...
def test_idempotent_and_noop_on_empty():
    bulk = _synthetic_bulk()
    once = seed_bulk_species(bulk, _drug_specs())
    twice = seed_bulk_species(once, _drug_specs())
    assert list(twice["id"]) == list(once["id"])          # no duplication
    assert list(once["id"]).count("drugX[p]") == 1
