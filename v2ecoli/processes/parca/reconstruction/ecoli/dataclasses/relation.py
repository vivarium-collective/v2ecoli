...
from v2ecoli.processes.parca.reconstruction.ecoli.dataclasses.process.replication import (
    MAX_TIMESTEP,
)
from v2ecoli.processes.parca.wholecell.utils import units
from v2ecoli.processes.parca.wholecell.utils.polymerize import polymerize
...
class Relation(object):
    ...
    def _build_codon_sequences(self, raw_data, sim_data):
        ...
        rna_sequences = sim_data.getter.get_sequences(
            [rna_id[:-3] for rna_id in sim_data.process.transcription.rna_data["id"]]
        )

        # Selenocysteine (single letter code: U) is encoded by opal codon (UGA)
        coding_opal = {}
        for id, seq in zip(protein_ids, protein_sequences):
            if "U" in seq:
                coding_opal[str(id)[:-3]] = seq.index("U")

        # Convert from transcription unit sequences to cistron sequences
        cistron_bounds = dict(
            sorted(sim_data.process.transcription.cistron_start_end_pos_in_tu.items())
        )
        ...
        for (cistron_idx, tu_idx), (start, end) in cistron_bounds.items():
            ...
            if (
                cistron_sequences[cistron_idx] != 0
                and cistron_sequences[cistron_idx] != curr_sequence
            ):
                cistron_id = sim_data.process.transcription.cistron_data["id"][
                    cistron_idx
                ]
                tu_id = sim_data.process.transcription.rna_data["id"][tu_idx]
                warnings.warn(
                    f"mRNA sequence for cistron {cistron_id}"
                    f" does not match mRNA sequence derived from TU {tu_id}."
                )
        ...
        for i, protein_id in enumerate(protein_ids):
            ...
            if protein_id in coding_opal:
                ...
                rna_sequence_translated = (
                    rna_sequence_translated[:opal]
                    + "U"
                    + rna_sequence_translated[opal + 1 :]
                )
    ...
    def _build_codon_dependent_trna_charging(self, raw_data, sim_data):
        ...
        def is_base_pair(codon, anticodon):
            ...
            for i, (C, A) in enumerate(zip(codon, anticodon[::-1])):
                ...
                if i == 2:
                    return np.any(
                        [watson_crick_base_pairing[A] == C, wobble_base_pairing[A] == C]
                    )
        ...
        # Base pairing rules
        watson_crick_base_pairing = {"A": "U", "U": "A", "G": "C", "C": "G"}
        wobble_base_pairing = {"A": "", "U": "G", "G": "U", "C": ""}

        # Map tRNAs to their anticodons
        cistron_data = sim_data.process.transcription.cistron_data
        free_trnas = np.array(
            [f"{trna}[c]" for trna in cistron_data["id"][cistron_data["is_tRNA"]]]
        )
        ...
        for i, amino_acid in enumerate(sim_data.molecule_groups.amino_acids):
            ...
            for codon in codons:
                ...
                if len(self.codon_to_trnas[codon]) == 0:
                    print(
                        f"Warning: No tRNAs were found to read codon {codon}."
                        f" Assigning all trnas for {amino_acid} to this codon."
                    )
                    self.codon_to_trnas[codon] = [
                        free_to_charged[trna] for trna in trnas
                    ]
        ...
        self.modified_aa_from_synthetase = np.copy(transcription.aa_from_synthetase)
    ...
    def _build_trna_charging_kinetics(self, raw_data, sim_data):
        ...
        for row in raw_data.optimization.trna_charging_kinetics_solutions:
            ...
            if (
                synthetase
                not in self.trna_charging_kinetics[sweep_level][
                    "synthetase_to_objective"
                ]
            ):
                ...
                row["objective"]
                < self.trna_charging_kinetics[sweep_level]["synthetase_to_objective"][
                    synthetase
                ]
            ):
                update(sweep_level, row)
        ...
        self.synthetase_to_k_cat = self.trna_charging_kinetics[default_sweep_level][
            "synthetase_to_k_cat"
        ]
        self.synthetase_to_K_A = self.trna_charging_kinetics[default_sweep_level][
            "synthetase_to_K_A"
        ]
        self.trna_to_K_T = self.trna_charging_kinetics[default_sweep_level][
            "trna_to_K_T"
        ]
    ...
    def optimize_trna_charging_kinetics(self, sim_data, cell_specs, amino_acid):
        ...
        def objective(
            ...
            v_charge = k_cat * c_synthetase * saturation_amino_acid * saturation_trnas
            ...
            # Steady-state cost
            steady_state_error = np.sum(np.square(1 - (v_charge / v_usage)))

            ############################################################

            # Calculate charging rate of each free trna
            relative_trnas = (
                (maps["f_to_cases"] @ min_f) * c_trnas / (maps["K_T_to_cases"] @ K_T)
            )
            trna_sum = maps["cases_to_trna_sum"] @ relative_trnas
            saturation_trnas = relative_trnas / (1 + trna_sum)
            v_charge_min = (
            ...
            # Steady-state cost
            min_steady_state_error = np.sum(np.square(1 - (v_charge_min / v_usage_min)))
        ...
        def full_saturation_amino_acid(c_amino_acid, c_synthetase, v_codons_total):
            """
        ...
        def get_random_initial_solution(
            ...
            if assume_full_saturation:
                ...
                K_A = np.random.uniform(
                    np.min(np.log10(c_amino_acid)), np.max(np.log10(c_amino_acid)), 1
                )[0]
    ...
    def get_constants(
        ...
        for condition in self.conditions:
            ...
            c_amino_acid = (
                sim_data.process.metabolism.concentration_updates.concentrations_based_on_nutrients(
                    media_id=media_id
                )[amino_acid]
            ).asNumber(self.conc_unit)

            # Get codon reading rate
            v_codons = (sim_data.codon_read_rate[media_id][i_codons]).asNumber(
                self.rate_unit
            )
            ...
            trna_TU_idx = np.take(
                sorter,
                np.searchsorted(container["id"], trna_TUs, sorter=sorter),
                mode="clip",
            )
    ...
    def assign_K_T(self, trnas, codons):
        ...
        trna_groups = list(set([tuple(self.codon_to_trnas[codon]) for codon in codons]))
