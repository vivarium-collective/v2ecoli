    ...
    def _prepare(self, states):
        ...
        if states["full_chromosomes"]["_entryState"].sum() > 0:
            ...
            if has_ppgpp:
                ...
                # The guard is a floating-point knife-edge: vEcoli's basal for
                # such genes is denormal-tiny (so scale stays ~0 and the delta
                # is suppressed), while a different BLAS/LAPACK build flushes it
                # to exact 0.0 and trips the guard — making one null gene reach
                # ~19% of all transcription on carbon-poor media and diverging
                # from vEcoli. Scale the delta by the real (near-zero) basal so
                # null genes stay suppressed regardless of underflow.
            else:
