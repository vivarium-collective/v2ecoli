...
if TYPE_CHECKING:
    from v2ecoli.processes.parca.wholecell.utils._netflow._base import NetworkFlowProblemBase
...
class FluxBalanceAnalysis(object):
    ...
    ## Objective
    _generatedID_moleculesToEquivalents = (
        "molecules of to fractional objective equivalents - "
    )
    _generatedID_moleculeEquivalents = (
        "fractional objective equivalent for molecules of - "
    )

    ## Flex FBA
    _generatedID_fractionalDifferenceLeading = (
        "difference between fractional objective equivalents of leading molecule and - "
    )
    ...
    _generatedID_kineticReactionEquivalentsPseudoflux = (
        "pseudoflux to remove kinetic objective equivalents for reaction - "
    )
    _generatedID_kineticReactionEquivalents = (
        "kinetic reaction objective equivalent for reaction - "
    )

    ## Kinetic
    _generatedID_reactionFluxEquivalents = "reaction flux equivalent for reaction - "
    _generatedID_conversionFlux = (
        "Flux converting flux to kinetic objective equivalents - "
    )
    ...
    def __init__(
        ...
        if objectiveParameters is not None:
            self.kineticObjectiveWeight = objectiveParameters.get(
        ...
        self._kinetic_objective_scaling = 1.0  # Value to scale by time step
        ...
        if self.objectiveType == "standard":
            ...
            if "reactionRateTargets" in objectiveParameters:
                ...
                raise FBAError(
                    "When using homeostatic_kinetics_mixed objective, a reactionRateTargets dict of reactionName:target rate must be provided in objectiveParameters."
                )
            ...
            raise FBAError(
                "Unrecognized self.objectiveType: {}".format(self.objectiveType)
            )

        self._initInternalExchange(internalExchangedMolecules)

        self._initExchangeMass(
            externalExchangedMolecules, moleculeMasses, secretionPenaltyCoeff
        )
    ...
    def _initReactionNetwork(self, reactionStoich):
        ...
        for reactionID in sorted(reactionStoich):
            ...
            for moleculeID, stoichCoeff in stoichiometry.items():
                self._solver.setFlowMaterialCoeff(reactionID, moleculeID, stoichCoeff)
    ...
    def _initExternalExchange(self, externalExchangedMolecules):
        ...
        for moleculeID in externalExchangedMolecules:
            ...
            if moleculeID in externalMoleculeIDs:
                print(
                    "Warning: attempting to set multiple external exchanges"
            ...
            self._solver.setFlowMaterialCoeff(exchangeID, moleculeID, -1)
    ...
    def _initObjectiveEquivalents(
        self, objective, pseudoFluxFormat=None, objectiveEquivFormat=None
    ):
        ...
        for moleculeID in sorted(objective):
            ...
            self._solver.setFlowMaterialCoeff(pseudoFluxID, moleculeID, -coeff)

            self._solver.setFlowMaterialCoeff(pseudoFluxID, objectiveEquivID, +1)
    ...
    def _initObjectiveStandard(self, objective):
        ...
        for moleculeID in objective:
            ...
            self._solver.setFlowMaterialCoeff(
                self._standardObjectiveReactionName, objectiveEquivID, -1
            )

        self._solver.setFlowObjectiveCoeff(self._standardObjectiveReactionName, +1)
    ...
    def _initObjectiveFlexible(self, objective, objectiveParameters):
        ...
        if any(coeff < 0 for coeff in objective.values()):
            warnings.warn(
                "flexFBA is not designed to use negative biomass coefficients"
            )

        # Add biomass to objective
        self._solver.setFlowObjectiveCoeff(
            self._standardObjectiveReactionName, biomassSatisfactionWeight
        )
        ...
        for moleculeID in objective:
            ...
            fractionDifferenceLeadingID = (
                self._generatedID_fractionalDifferenceLeading + moleculeID
            )

            self._solver.setFlowMaterialCoeff(
                leadingMoleculeToFractionID, fractionDifferenceLeadingID, +1
            )

            moleculeToFractionID = self._generatedID_moleculesToEquivalents + moleculeID

            self._solver.setFlowMaterialCoeff(
                moleculeToFractionID, fractionDifferenceLeadingID, -1
            )

            fractionDifferenceLeadingOutID = (
                self._generatedID_fractionalDifferenceLeadingOut + moleculeID
            )

            self._solver.flowMaterialCoeffIs(
                fractionDifferenceLeadingOutID, fractionDifferenceLeadingID, -1
            )

            self._solver.setFlowObjectiveCoeff(
                fractionDifferenceLeadingOutID, -fractionalDifferenceWeight
            )
        ...
        for moleculeID in objective:
            fractionDifferenceBiomassID = (
                self._generatedID_fractionalDifferenceBiomass + moleculeID
            )

            moleculeToFractionID = self._generatedID_moleculesToEquivalents + moleculeID

            self._solver.setFlowMaterialCoeff(
                moleculeToFractionID, fractionDifferenceBiomassID, +1
            )

            self._solver.setFlowMaterialCoeff(
                fractionDifferenceBiomassID, self._standardObjectiveReactionName, -1
            )

            fractionDifferenceBiomassOutID = (
                self._generatedID_fractionalDifferenceBiomassOut + moleculeID
            )

            self._solver.setFlowMaterialCoeff(
                fractionDifferenceBiomassOutID, fractionDifferenceBiomassID, -1
            )
    ...
    def _initObjectiveHomeostatic(self, objective):
        ...
        if any(coeff < 0 for coeff in objective.values()):
            raise FBAError(
                "Homeostatic FBA is not designed to use negative biomass coefficients"
            )
    ...
    def _initObjectiveRangeHomeostatic(self, objective, objectiveParameters):
        ...
        homeostaticRangeObjFractionHigher = objectiveParameters.get(
            "homeostaticRangeObjFractionHigher", 0
        )
    ...
    def _initObjectiveKinetic(self, objective, objectiveParameters=None):
        ...
        self._oneSidedReactions = set(
            objectiveParameters.get("oneSidedReactionTargets", "")
        )
        ...
        for reactionID, expectedFlux in objective.items():
            if expectedFlux < 0:
                raise FBAError(
                    "Target flux for reaction {} is negative. Kinetic targets must be positive - set the value for the (reverse) reaction if a negative flux is desired.".format(
                        reactionID
                    )
                )
            ...
            if reactionID not in self.reactionStoich:
                raise FBAError(
                    "{} is not in the reaction network. Target fluxes must be in the reaction network".format(
                        reactionID
                    )
                )
            ...
            reactionFluxEquivalent = (
                self._generatedID_reactionFluxEquivalents + reactionID
            )
            ...
            kineticObjEquivalent = (
                self._generatedID_kineticReactionEquivalents + reactionID
            )
            self._solver.setFlowMaterialCoeff(
                conversionFlux,
                reactionFluxEquivalent,
    ...
    def set_scaled_kinetic_objective(self, scaling=None, reactions=None):
        ...
        if scaling is None:
            ...
            raise ValueError(
                "Not able to set a different non-zero scaling factor"
        ...
        if internalExchangedMolecules is not None:
            for moleculeID in internalExchangedMolecules:
                ...
                if moleculeID in internalMoleculeIDs:
                    print(
                        "Warning: attempting to set multiple internal exchanges"
                        + " for {}. Skipping...".format(moleculeID)
                    )
                    continue
    ...
    def _initExchangeMass(
        self, externalExchangedMolecules, moleculeMasses, secretionPenaltyCoeff
    ):
        ...
        if moleculeMasses is not None:
            self._solver.setFlowMaterialCoeff(
                self._massExchangeOutName, self._massExchangeID, -1
            )
            ...
            for moleculeID in externalExchangedMolecules:
                ...
                try:
                    ...
                    raise FBAError(
                        "You must provide masses for all molecules in externalExchangedMolecules"
                    )
                ...
                if secretionPenaltyCoeff is not None:
                    ...
                    self._solver.setFlowMaterialCoeff(
                        exchangeFluxID, absID, moleculeMass
                    )

                    self._solver.setFlowMaterialCoeff(posExchangeID, absID, -1)

                    self._solver.setFlowMaterialCoeff(negExchangeID, absID, +1)

                    self._solver.setFlowObjectiveCoeff(
                        posExchangeID, secretionPenaltyCoeff
                    )
    ...
    def _initMaintenance(self, maintenanceCostGAM, maintenanceReaction):
        ...
        if (maintenanceCostGAM is None) ^ (maintenanceReaction is None):
            raise FBAError(
                "Must pass all or none of maintenanceCostGAM, maintenanceReaction"
            )
        ...
        self._solver.setFlowMaterialCoeff(
            self._massExchangeOutName, self._pseudometaboliteGAM, maintenanceCostGAM
        )

        # ... which are consumed in a separate flux

        self._solver.setFlowMaterialCoeff(
            self._reactionID_GAM, self._pseudometaboliteGAM, -1
        )

        for moleculeID, stoichCoeff in maintenanceReaction.items():
            self._solver.setFlowMaterialCoeff(
                self._reactionID_GAM, moleculeID, stoichCoeff
            )

            self._solver.setFlowMaterialCoeff(
                self._reactionID_NGAM, moleculeID, stoichCoeff
            )

            self._solver.setFlowMaterialCoeff(
                self._reactionID_polypeptideElongationEnergy, moleculeID, stoichCoeff
            )
    ...
    def setExternalMoleculeLevels(
        self, levels, molecules=None, force=False, allow_export=False
    ):
        """
        ...
        for moleculeID, level in zip(molecules, levels_array):
            ...
            if level < 0:
                if not allow_export:
                    print(
                        "Setting a negative external molecule level for {} - be sure this is intended behavior.".format(
                            moleculeID
                        )
                    )
    ...
    def setReactionFluxBounds(
        self, reactionIDs, lowerBounds=None, upperBounds=None, raiseForReversible=True
    ):
        """
        ...
        for reactionID, lb, ub in zip(reactionIDs, lowerBounds, upperBounds):
            if (
                reactionID not in self._reactionIDsSet
                and reactionID not in self._specialFluxIDsSet
            ):
                raise InvalidBoundaryError(
                    "Unable to set reaction flux: reaction '%s' not recognized."
                    % (reactionID,)
                )
        ...
        for molecule_id, coeff in objective.items():
            ...
            if molecule_id not in self._outputMoleculeIDs:
                raise FBAError(
                    ...
                    + "not adding new ones. %s is an unrecognized molecule"
                    % molecule_id
                )

            pseudo_flux_id = self._generatedID_moleculesToEquivalents + molecule_id

            self._solver.setFlowMaterialCoeff(pseudo_flux_id, molecule_id, -coeff)
    ...
    def getOutputMoleculeIDs(self):
        return tuple(
            k for k, v in sorted(self._outputMoleculeIDs.items(), key=lambda d: d[1])
        )
    ...
    def getBiomassReactionFlux(self):
        if self.objectiveType not in ("standard", "flexible", "moma"):
            raise FBAError(
                "There is no biomass reaction for this objective type ({})".format(
                    self.objectiveType
                )
            )
    ...
    def getHomeostaticObjectiveValues(self, moleculeIDs=None):
        ...
        for idx, moleculeID in enumerate(moleculeIDs):
            if moleculeID not in self._homeostaticTargetMolecules:
                raise FBAError(
                    "No homeostatic target set for molecule {}.".format(moleculeID)
                )
    ...
    def getKineticReactionFluxTargets(self, reactionIDs=None):
        ...
        for idx, reactionID in enumerate(reactionIDs):
            if reactionID not in self._currentKineticTargets:
                raise FBAError(
                    "No kinetic target set for reaction {}.".format(reactionID)
                )
    ...
    def getKineticTargetFluxErrors(self, reactionIDs=None):
        if reactionIDs is None:
            reactionIDs = self.getKineticTargetFluxNames()
        errors = self.getReactionFluxes(
            reactionIDs
        ) - self.getKineticReactionFluxTargets(reactionIDs)
        # Adjust for any one-sided reactions
        if len(self._oneSidedReactions) > 0:
            oneSidedNegativeErrors = np.where(
                [
                    True
                    if reactionID in self._oneSidedReactions and errors[idx] < 0
                    else False
                    for idx, reactionID in enumerate(reactionIDs)
                ]
            )
    ...
    def getKineticTargetFluxRelativeDifferences(self, reactionIDs=None):
        if reactionIDs is None:
            reactionIDs = self.getKineticTargetFluxNames()
        return self.getKineticTargetFluxErrors(
            reactionIDs
        ) / self.getKineticReactionFluxTargets(reactionIDs)
    ...
    def getKineticObjectiveValues(self, reactionIDs=None):
        ...
        fluxes = {
            rxn: flux
            for rxn, flux in zip(generated_ids, self.getReactionFluxes(generated_ids))
        }
        ...
        for idx, reactionID in enumerate(reactionIDs):
            ...
            if self._solver.quadratic_objective:
                ...
                relax = (
                    fluxes[quadUnityID] ** 2
                    + self.kinetic_objective_weight_in_range
                    * fluxes[quad_range_id] ** 2
                )
            else:
    ...
    def setKineticTarget(
        self, reactionIDs, mean_targets, lower_targets=None, upper_targets=None
    ):
        ...
        def validate_targets(targets, reactionIDs, default=None):
            ...
            if (np.array(targets) < 0).any():
                raise FBAError(
                    "Rate targets cannot be negative. {} were provided with targets of {}".format(
                        np.array(reactionIDs)[np.array(targets) < 0],
            ...
            if len(targets) != len(reactionIDs):
                raise FBAError(
                    "There must be equal numbers of reactionIDs and"
        ...
        lower_targets = validate_targets(
            lower_targets, reactionIDs, default=mean_targets
        )
        upper_targets = validate_targets(
            upper_targets, reactionIDs, default=mean_targets
        )

        # Change the objective normalization
        for reactionID, mean, lower, upper in zip(
            reactionIDs, mean_targets, lower_targets, upper_targets
        ):
            if reactionID not in self._kineticTargetFluxes:
                raise FBAError(
                    "Kinetic targets can only be set for reactions"
                    " initialized to be kinetic targets. {} is not set up for it.".format(
                        reactionID
                    )
                )

            if (upper < mean and not np.isclose(upper, mean)) or (
                mean < lower and not np.isclose(mean, lower)
            ):
                raise FBAError(
                    "Incorrect ordering of kinetic targets: {} {} {}."
            ...
            if mean == 0:
                ...
                self._solver.setFlowMaterialCoeff(
                    conversionFlux, reactionFluxEquivalent, -1
                )
                ...
                self._solver.setFlowMaterialCoeff(
                    conversionFlux, reactionFluxEquivalent, -mean
                )
    ...
    def solve(self, iterations):
        if iterations == 0:
            ...
            try:
                ...
                print(
                    "Warning: {} error while solving FBA - repeating FBA solve".format(
                        inst
                    )
                )
