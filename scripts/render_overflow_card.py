"""Render the metabolic-perturbation **overflow** report card.

A *perturbation* report card (sibling of the basal `vs_literature` card): grade
the model's aerobic acetate-overflow response — acetate-carbon yield vs growth
rate — against curated chemostat/batch references, with the same reference-agnostic
grader (the ``curve_response`` criterion) pointed at a swept response.
...
graded behavior; the onset is reported but secondary (it shifts strain-to-strain).
...
The model arm comes from the GUR-titration runner (``run_gur_titration.py`` →
``gur_titration.json``): per cap, the emergent growth rate + acetate + glucose
exchange, baked so the card stays sweep-independent.
"""
...
OUT = REPO / "docs/report_cards/metabolic_perturbation/overflow_acetate_vs_growth"
...
_MODEL_JSON = OUT / "model_overflow_baseline.json"
...
def build_overflow(model: dict, bundle_path: Path | None = None) -> tuple[dict, dict]:
    ...
    Vemuri curve (curve_response), with Basan + the MG1655 anchor as context."""
    ...
    axes = {"metabolism.acetate_overflow": {
        ...
        "how": ("Model: acetate-carbon yield Y_ac = (2·acetate)/(6·glucose) at the "
                ...
                "curve_response criterion: the response shape (Y_ac rising ~0 → "
                "linear above an onset) and the yield slope; the onset is reported "
                "but secondary (it shifts strain-to-strain). Basan 2015 (NCM3722 "
                "batch, SI-derived yield) and an independent MG1655 ¹³C yield point "
                "are overlaid as context."),
        "criterion": {
            "type": "curve_response", "ref_x": ref["x"], "ref_y": ref["y"],
...
def build(model_json: Path = _MODEL_JSON, bundle_path: Path | None = None) -> tuple[dict, dict, dict]:
    """Return (card, reference, model) — importable for tests."""
    ...
    reference = {
        "title": "Metabolic perturbation — aerobic acetate overflow vs growth rate",
        "status": "populated",
        "stimulus": {
        ...
        "findings": [
            "Perturbation card: the model's acetate-overflow response is graded as a "
            "dimensionless acetate-carbon yield vs growth rate (per-OD vs per-gDW "
            "cancels), shape-first — the 0→linear rise and its slope, onset secondary.",
        ...
        "footer": "Behavioral report card (curve_response grader) · metabolic perturbation · overflow.",
        "axes": ax,
    }
