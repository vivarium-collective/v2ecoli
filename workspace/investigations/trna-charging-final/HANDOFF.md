...
| 3b | Port `__init__` parameter unpacking | 1 session, 1.5–2 hr | Replace the `initialize` stub with the upstream ~80 LOC of parameter unpacking (lines 2208–2282). Constants, codon-sequence tables, slice indexes for the molecules buffer, mapping arrays (`trnas_to_amino_acids/codons`, `codons_to_amino_acids`), `trnas_to_amino_acid_indexes`, kinetic parameters, `max_attempts`, `reconciliation_buffer`, `previous_rate`. Smoke-test instantiation against a hand-built parameters dict. |
| 3c | Port `elongation_rate` + `request` + helpers | 1 session, 2 hr | Wires `kernel.get_elongation_rate` into `elongation_rate`; `run_model` does pre-allocation prediction; `request` calls `run_model("bulk_total")` and emits requests for AAs / ATP / tRNAs / synthetases / MAP. |
...
- **Session 9 (next):** Task #3b — port the upstream `KineticTrnaChargingModel.__init__` (~80 LOC). All 12 new `config_schema` keys are in place; 3b unpacks them into instance attrs, builds the slice indexes, derives `trnas_to_amino_acid_indexes`, sets `previous_rate`. Drop the `Task 3b` markers from the `initialize` and `get_kinetic_constants` stubs; the scaffold's marker tests will then fail for those two methods (signaling 3b is the right session to update them).
- **Session 10:** Task #3c — `elongation_rate`, `request`, `run_model`, `codon_sequences_width`, `sequences`, `max_charging_rate`.
