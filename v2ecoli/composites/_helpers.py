...
def _get_special_step(loader, step_name, core):
    ...
    if step_name == 'dnaa-adp-release':
        # dnaa-2 Step 3 (Haochen 2026-05-31): slow NON-equilibrium DnaA-ADP ->
        # apo-DnaA release (k_r ~1e-7/s ≈ 6e-6/min). Pairs with zeroing
        # MONOMER0-4565_RXN in the equilibrium cache (see
        # scripts/patch_dnaa_adp_nonequilibrium.py) so DnaA-ADP is formed only
        # by hydrolysis and drained only here. Rate overridable for tuning.
        import os
        rate = float(os.environ.get('DNAA_ADP_RELEASE_RATE', '6e-6'))
        from v2ecoli.steps.dnaa_adp_release import DnaaAdpRelease
        instance = DnaaAdpRelease(config={'rate_per_min': rate}, core=core)
        topology = getattr(instance, 'topology', {})
        return instance, topology, 'step'
