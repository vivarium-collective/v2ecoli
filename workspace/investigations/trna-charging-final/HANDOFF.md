...
- **Session 8 (next):** Task #3 (`KineticTrnaChargingModel` class + `kinetic_charging_baseline` composite architecture + behavior test). The kernel module is ready to consume — see `v2ecoli/processes/polypeptide/kinetic_charging_kernel.py`. Implement the `KineticTrnaChargingModel` (upstream at `polypeptide_elongation.py:2198`) as a sibling to `SteadyStateElongationModel` inside `v2ecoli/processes/polypeptide/`. Register the new arch via `@composite_generator(name="kinetic_charging_baseline", ...)` in `v2ecoli/composites/kinetic_charging_baseline.py` and append the import to `v2ecoli/composites/__init__.py`. Update `v2ecoli/library/cache_version.py:INPUT_FILES` accordingly. Behavior test goes in `tests/test_behavior_kinetic_charging.py`.
