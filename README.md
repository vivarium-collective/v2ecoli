...
- **[Nutrient-Growth Report](https://vivarium-collective.github.io/v2ecoli/nutrient_growth_report.html)** — 
  Continuous glucose-uptake kinetics (Michaelis-Menten) + environment depletion feedback
  + mass-conservation enforcement via a dark-matter pool. Single-cell run in a 10 fL
  environment shows exp→stat transition, exchange-flux diagnostics, carbon and mass
  balance, and Caglar 2017 calibration targets. Features are opt-in via
  `V2ECOLI_NUTRIENT_GROWTH=1` / `V2ECOLI_DARK_MATTER=1`; defaults match vEcoli 1.0.
...
report and generated by `python compare_report.py` or, individually, by
`python network_report.py --model baseline|departitioned|reconciled`.

### Generating reports

```bash
python workflow.py                               # Workflow Report
python colony_report.py                          # Colony Report
python compare_report.py                         # Architecture Comparison (3-way v2ecoli)
python compare_v1_v2.py                          # Engine Comparison (v1 / v2 / v2ecoli)
python nutrient_growth_report.py                 # Nutrient-Growth (v1-compat defaults)
python nutrient_growth_report.py --dark-matter   # … with mass-balance enforcement
```
