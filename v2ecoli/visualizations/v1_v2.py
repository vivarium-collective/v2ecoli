...
# ---------------------------------------------------------------------------
_ENGINES = [
    ("vecoli_v1",         "vEcoli (vivarium)",          "#3b82f6", "--"),
    ("v2ecoli",           "v2ecoli (process-bigraph)",  "#ef4444", "-"),
]
...
def _render_comparison_html(datasets: dict, duration: float, title: str) -> str:
    ...
    # Table column headers are derived from _ENGINES so the report tracks
    # exactly the engines being compared (no hardcoded column set).
    engine_headers = "".join(f"<th>{escape(label)}</th>" for _, label, _, _ in _ENGINES)
...
"""
        ...
        "<p>Runtime and growth-behavior comparison of <b>vEcoli</b> — the established "
        "vivarium-based <i>E. coli</i> whole-cell model — and <b>v2ecoli</b>, the current "
        "process-bigraph baseline. Each engine runs from its own ParCa-generated "
        "<code>sim_data</code> (vEcoli's regenerated from current master; v2ecoli's from its "
        "own cache) over the same simulated duration, so the comparison reflects engine and "
        "runtime differences rather than a re-parameterization of the biology.</p>\n"
        "<h2>Performance</h2>\n"
        f'<div class="perf">\n{perf_cards}</div>\n'
        "<table>\n"
        f"<tr><th>Metric</th>{engine_headers}</tr>\n"
        f"{overview_rows}\n"
        "</table>\n"
    ...
    html += (
        ...
        "<table>\n"
        f"<tr><th>Time</th>{engine_headers}</tr>\n"
        f"{mass_table_rows}\n"
        "</table>\n"
        "<footer style='margin-top:2em;color:#64748b;font-size:0.85em;'>"
        "Generated by V1V2Visualization &middot; vEcoli vs v2ecoli"
        "</footer>\n"
    )
...
class V1V2Visualization(Visualization):
    ...
    def update(self, state: dict[str, Any]) -> dict:
        ...
        title = self.config.get("title")       or "vEcoli vs v2ecoli"
        ...
        # Per-engine runtime (wall/sim/load/speed) is threaded via
        # meta["runtime"][engine_key]; merge it into each engine's meta so the
        # performance table is populated (snapshots alone carry no timing).
        rt = meta.get("runtime", {}) if isinstance(meta, dict) else {}
        datasets: dict[str, Any] = {
            "vecoli_v1":        {"snapshots": h_v1,  **_meta_from_rows(h_v1,  {**meta, **rt.get("vecoli_v1", {})})},
            "vecoli_composite": {"snapshots": h_v2,  **_meta_from_rows(h_v2,  {**meta, **rt.get("vecoli_composite", {})})},
            "v2ecoli":          {"snapshots": h_v2e, **_meta_from_rows(h_v2e, {**meta, **rt.get("v2ecoli", {})})},
        }
