...
- **DONE — real paired e2e (mini):** the candidate arm runs end-to-end through the real production worker path (spawned `EnvWorker` → `run_study` → real `ecoli_baseline` sim → real `runs.db` rows), and a full paired `basal` run through `run_investigation_composite` wrote **completed rows for BOTH arms** (candidate `ecoli_baseline` + reference `vecoli`), verdicts `within_tol`, `regression_compatibility: PASS`. This is the in-vivo proof; the hermetic tests above gate the build.
- **Reference-side vEcoli checkout (use a self-consistent one).** The reference (`vecoli`) arm runs genuine upstream vEcoli at `$V2E_VECOLI_DIR`. A `MonomerCounts` shape crash — `ValueError: operands could not be broadcast together with shapes (45,) (41,)` at `ecoli/processes/listeners/monomer_counts.py` — appears **only in a split checkout** that has CovertLab commit `14f04a3f` (listener `two_component_system_molecule_ids` → `.modified_molecules`, 45) *without* its same-day pair `2208460b` (which expands `TwoComponentSystem.stoich_matrix_monomers()` rows to `.modified_molecules` too, so the stoich matrix is also 45). Both landed May 1 2026 and **current CovertLab `master` has both → self-consistent, no crash**. Fix = point `$V2E_VECOLI_DIR` at a checkout that contains **both** commits (e.g. current CovertLab `master`), or one that predates both; do **not** "pin before `14f04a3f`" alone. (A vivarium-collective fork branch stuck between the two commits is the trap — update it past `2208460b`.) External-repo requirement; does not gate this build's hermetic tests.
